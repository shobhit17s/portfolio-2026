...
(function (HW) {
  ...
  /* ---------- how a phone is framed ----------
     Two numbers, and they pull against each other.

     NARROW_BACK is how far the camera stands off. Low and the planet fills
     the width but its moons are cropped off the sides - and on the Drafting
     Fields a cropped moon is a case study you cannot tap. High and the
     planet shrinks to a marble.

     NARROW_LIFT is how far up the screen the planet sits, as a fraction of
     the height, to leave the bottom for the writing. Too much and there is a
     lake of empty sky between the planet and the spin dial; too little and
     the top moon disappears behind the name bar. */
  var NARROW_BACK = 1.55;
  var NARROW_LIFT = 0.11;
  ...
  HW.createScene = function (canvas, worlds) {
    ...
    function fitOverflow(fit) {
      ...
      var cY = H / 2 + (narrow ? -H * NARROW_LIFT : A.lift * H);
    ...
    function layoutCamera(time) {
      ...
      var narrow = W < 820;
      if (narrow) {
        /* Phones: stand back a little and sit the planet in the upper half,
           so the writing has the bottom of the screen to itself.

           This number is a balance. Too high and the planet shrinks to a
           marble with a lake of empty sky under it; too low and its
           neighbours are cropped off the sides. 1.32 keeps the world you are
           on filling most of the width, which is what makes a phone feel
           like you are standing somewhere rather than looking at a diagram. */
        var away = NARROW_BACK;
      ...
      cam.shiftY = narrow ? -H * NARROW_LIFT : HW.lerp(A.lift, B.lift, e) * H;
