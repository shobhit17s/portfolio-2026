...
(function (CS) {
  ...
  CS.initDrawer = function () {
    ...
    function open(id) {
      ...
      /* A shared panel is words only - no picture, no caption. Hide the
         picture and its caption rather than leaving the last one showing,
         or printing the word "undefined" under an empty frame. */
      var hasMedia = !!d.media;
      imgEl.parentNode.hidden = !hasMedia;
      if (hasMedia) CS.PrototypeEmbed.fill(imgEl, d.media, CS.PrototypeEmbed.lookup(d.media));
      capEl.textContent = d.caption || '';
      textEl.innerHTML = (d.body || []).map(function (p) { return '<p>' + p + '</p>'; }).join('');
      if (d.note) textEl.insertAdjacentHTML('beforeend', '<p class="cs-meta">' + d.note + '</p>');
